...
public extension UIViewController {
    ...
    func alert(_ title: String?, message: String?) {
        self.alert(title, message: message, cancel: nil, buttons: [], completion: nil)
    }

    /// Title with message, 'Done' button and completion block
    func alert(_ title: String?, message: String?, completion: (() -> Void)?) {
        self.alert(title, message: message, cancel: nil, buttons: [], completion: { _ in
            if completion != nil {
    ...
    func alert(_ title: String?, message: String?, cancel: String?) {
        self.alert(title, message: message, cancel: cancel, buttons: [], completion: nil)
    }
    ...
    func alert(_ title: String?, message: String?, cancel: String?, completion: (() -> Void)?) {
        self.alert(title, message: message, cancel: cancel, buttons: [], completion: { _ in
            if completion != nil {
                DispatchQueue.main.async {
                    completion?()
                }
    ...
    func alertA(
        ...
        var cancelTitle: String = DataConstants.okText
    ...
    func actionSheet(
        ...
        var cancelTitle: String = DataConstants.cancelText
        if let cancel = cancel {
            cancelTitle = cancel
        }
        ...
        buttons.enumerated().forEach { buttonIndex, buttonTitle in
        ...
        guard UIDevice.isPhone else {
            ...
            alert.popoverPresentationController?.sourceRect = sourceRect ?? .init()
    ...
    func alert(
        ...
        alertController.addAction(
            UIAlertAction(title: reloadTitle, style: UIAlertAction.Style.default) { _ -> Void in
                DispatchQueue.main.async {
                    reload()
                }
            }
