...
open class ClusterManager {
     ...
     */
    open func reload(mapView: MKMapView, completion: @escaping (Bool) -> Void = { _ in }) {
    ...
    open func clusteredAnnotations(zoomScale: Double, visibleMapRect: MKMapRect, operation: Operation? = nil) -> (toAdd: [MKAnnotation], toRemove: [MKAnnotation]) {
        ...
        if self.shouldDistributeAnnotationsOnSameCoordinate {
            self.distributeAnnotations(tree: self.tree, mapRect: visibleMapRect)
        }

        let allAnnotations = self.dispatchQueue.sync {
            clusteredAnnotations(tree: tree, mapRects: mapRects, zoomLevel: zoomLevel)
        }

        guard !isCancelled else { return (toAdd: [], toRemove: []) }

        let before = self.visibleAnnotations
    ...
    func clusteredAnnotations(tree: QuadTree, mapRects: [MKMapRect], zoomLevel: Double) -> [MKAnnotation] {
        ...
        for mapRect in mapRects {
            ...
            if count >= self.minCountForClustering, zoomLevel <= self.maxZoomLevel {
                ...
                cluster.coordinate = self.coordinate(annotations: annotations, position: self.clusterPosition, mapRect: mapRect)
    ...
    func distributeAnnotations(tree: QuadTree, mapRect: MKMapRect) {
        let annotations = self.dispatchQueue.sync {
    ...
    func mapRects(zoomScale: Double, visibleMapRect: MKMapRect) -> [MKMapRect] {
        ...
        self.zoomLevel = zoomScale.zoomLevel
        let scaleFactor = zoomScale / self.cellSize(for: self.zoomLevel)
